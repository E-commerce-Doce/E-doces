...
class PedidoController extends Controller
{
    ...
    protected function listPedidos(string $msgErro = "", string $msgSucesso = "")
    {
        ...
        //Carregar a avaliaçao de cada pedido
        foreach ($pedidos as $pedido) {
            $pedido->setAvaliacaoObj($this->avaliacaoDao->findByPedido($pedido->getIdPedido()));
        }
    ...
    protected function realizarPagamento()
    {
        ...
        $pedidoPagamento = new Pedido();
        $pedidoPagamento->setIdPedido($dados['idPedido']);
        ...
        if (empty($erros)) {
            try {
                ...
                $this->pedidoDao->makePagamento($pedidoPagamento);
    ...
    protected function createPagamento()
    {
        ...
        // Busca os dados do pagamento e do confeiteiro
        $pagamento = $this->pedidoDao->findByIdCompleto($idPedido);
        ...
        // Renderiza a View com os dados completos
    ...
    public function cancelarPedido()
    {
        ...
        // Obtém o ID do pedido enviado via GET
        $idPedido = $_GET['idPedido'] ?? null;

        // Verifica se o ID do pedido foi informado
        if (!$idPedido) {
            $_SESSION['msgErro'] = "Erro: ID do pedido não foi informado.";
            header("Location: " . BASEURL . "/controller/PedidoController.php?action=listPedidos");
            exit();
        }

        // Busca o pedido no banco para obter os detalhes
        $pedido = $this->pedidoDao->findByIdCompleto($idPedido);

        // Atualiza o status do pedido para 'CANCELADO'
        $this->pedidoDao->updateStatus($idPedido, 'CANCELADO');
        $nomeCliente = $pedido->getUsuario()->getNome();

        // Redireciona o cliente para a página de pedidos
        header("Location: " . BASEURL . "/controller/PedidoController.php?action=acompanharStatus&idPedido=" . $idPedido);
        exit();
    }
    ...
    protected function acompanharStatus()
    {
        ...
        if (isset($_GET['idPedido'])) {
            ...
            if ($pedido) {
                ...
                $telefoneConfeiteiro = $this->pedidoDao->getTelefoneConfeiteiroByPedido($pedidoId);
